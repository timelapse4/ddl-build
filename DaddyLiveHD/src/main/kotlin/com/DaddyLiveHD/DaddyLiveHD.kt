...
class DaddyLiveHD : MainAPI() {
    ...
    override suspend fun loadLinks(
        ...
        for (folder in folders) {
            ...
            try {
                val streamDoc = app.get(streamUrl, headers = headers, referer = mainUrl).document
                    ...
                    if (!m3u8.isNullOrBlank()) {
                        callback(
                            ExtractorLink(
                                ...
                                isM3u8 = true,
                                headers = mapOf("Referer" to streamUrl, "Origin" to mainUrl)
                            )
                        )
                        return true
                    }
                ...
                val iframeSrc = streamDoc.selectFirst("iframe[src]")?.attr("src") ?: continue
                if (iframeSrc == streamUrl) continue
                val innerDoc = app.get(iframeSrc, headers = headers, referer = streamUrl).document
