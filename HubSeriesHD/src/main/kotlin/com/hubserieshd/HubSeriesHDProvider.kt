...
class HubSeriesHDProvider : MainAPI() {
    ...
    private fun locateAndTapPlayer(webView: WebView, viewWidthPx: Int, viewHeightPx: Int, ctx: Context?) {
        val script = """
            (function() {
                try {
        ...
        webView.evaluateJavascript(script) { result ->
            ...
            toast(ctx, "HubSeriesHD: iframe rect = $cleaned")

            val x = (if (fx != null) (fx * viewWidthPx) else viewWidthPx / 2f)
                .coerceIn(0f, viewWidthPx.toFloat() - 1f)
            val y = (if (fy != null) (fy * viewHeightPx) else viewHeightPx / 2f)
                .coerceIn(0f, viewHeightPx.toFloat() - 1f)
                    ...
                    wv.webViewClient = object : WebViewClient() {
                        override fun onPageFinished(view: WebView?, url: String?) {
                            ...
                            Handler(Looper.getMainLooper()).postDelayed({
                                view?.let { locateAndTapPlayer(it, viewWidthPx, viewHeightPx, ctx) }
                            }, tapDelayMs)
                            Handler(Looper.getMainLooper()).postDelayed({
                                view?.let { locateAndTapPlayer(it, viewWidthPx, viewHeightPx, ctx) }
                            }, tapDelayMs + 4000L)
